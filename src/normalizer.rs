...
fn normalize_bexpr(e: &ast::Expr, intermediates: &mut HashMap<String, Expr>) -> Bexpr {
	match e {
		...
		ast::Expr::Tuple(exprs) => Bexpr::Tuple(
			exprs
				.iter()
				.map(|e| normalize_bexpr(e, intermediates))
				.collect(),
		),
	...
	match e {
		...
		ast::Expr::Fby(fby) => {
			...
			Expr::Fby(
				...
				vec![normalize_expr(e2, intermediates)],
			)
		}
...
fn normalize_node(n: &ast::Node) -> Node {
	...
	for (name, _) in n.locals.iter() {
		intermediates.insert(
			name.clone(),
			Expr::Bexpr(Bexpr::Atom(Atom::Const(Const::Unit))),
		);
	}
	let mut body: Vec<Equation> = n
		.body
		.iter()
		.map(|eq| normalize_equation(eq, &mut intermediates))
		.collect();
